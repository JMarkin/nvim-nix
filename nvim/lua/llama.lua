...
function LLAMA.setup(cfg)
  if cfg then
    LLAMA.config = vim.tbl_deep_extend("force", LLAMA.config, cfg)
  end
  ...
  if LLAMA.config.enable_at_startup then
    LLAMA.enable()
  end
end
...
function LLAMA.fim_manual()
  ...
  LLAMA._fim(-1, -1, false, {}, false)
end

LLAMA.fim = funcs.debounce(function(pos_x, pos_y, is_auto, prev, use_cache)
  LLAMA._fim(pos_x, pos_y, is_auto, prev, use_cache)
end, 500)

function LLAMA._fim(pos_x, pos_y, is_auto, prev, use_cache)
  local pos = vim.api.nvim_win_get_cursor(0)
...
function LLAMA.fim_accept(accept_type)
  ...
  if can_accept and #content > 0 then
    ...
    if #content > 1 and accept_type == "full" then
      vim.api.nvim_buf_set_lines(0, pos_y, pos_y, false, vim.list_slice(content, 2, #content))
    end
  ...
  if state.fim_hint_shown and can_accept then
    ...
    pos_x = pos[2]
    pos_y = pos[1]
    LLAMA._fim(pos_x, pos_y, true, state.fim_data.content, true)
  end
end
